...
<div class="data_list">
		...
		<form name="myForm" class="form-search" method="post" action="student?action=search" style="padding-bottom: 0px">
				<button class="btn btn-success" type="button" style="margin-right: 50px;" onclick="javascript:window.location='student?action=preSave'">添加学生</button>
				<span class="data_search">
					...
					<option value="dorm" ${searchType eq "dorm"?'selected':'' }>宿舍</option>
					</select>
			...
			<table class="table table-striped table-bordered table-hover datatable">
				...
				<c:forEach  varStatus="i" var="student" items="${studentList }">
					<tr>
						...
						<td>${student.tel }</td>
						<td><button class="btn btn-mini btn-info" type="button" onclick="javascript:window.location='student?action=preSave&studentId=${student.studentId }'">修改信息</button>&nbsp;
							<button class="btn btn-mini btn-danger" type="button" onclick="studentDelete(${student.studentId })">删除信息</button></td>
					</tr>
				</c:forEach>
				</tbody>
			</table>
