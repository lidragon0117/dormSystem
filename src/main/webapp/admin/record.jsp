...
<div class="data_list">
		...
		<form name="myForm" class="form-search" method="post" action="record?action=search" style="padding-bottom: 0px">
				<span class="data_search" style="float:left">
					...
					<select id="buildToSelect" name="buildToSelect" style="width: 110px;position:fixed;right:21%">
					...
					<select id="searchType" name="searchType" style="width: 80px;position:fixed;right:15%">
					<option value="name">姓名</option>
					<option value="number" ${searchType eq "number"?'selected':'' }>学号</option>
					<option value="dorm" ${searchType eq "dorm"?'selected':'' }>寝室</option>
					</select>
					&nbsp;<input id="s_studentText" name="s_studentText" type="text"  style="width:120px;height: 30px;position:fixed;right:7%" class="input-medium search-query" value="${s_studentText }" placeholder="请输入~">
					&nbsp;<button type="submit" class="btn btn-info" style="position:fixed;right:2%" onkeydown="if(event.keyCode==13) myForm.submit()">查一下</button>
				</span>
		</form>
		<div>
