...
<div class="data_list">
		...
		<form name="myForm" class="form-search" method="post" action="record?action=search" style="padding-bottom: 0px">
				<button class="btn btn-success" type="button" style="margin-right: 50px;" onclick="javascript:window.location='record?action=preSave'">添加缺勤</button>
				<span class="data_search">
					...
					<span><font style="font-family: '黑体'; font-style: 'bold'; font-size: 20px" color="red">${dormBuildName }&nbsp;&nbsp;</font></span>
					...
					&nbsp;<input id="s_studentText" name="s_studentText" type="text"  style="width:120px;height: 30px;" class="input-medium search-query" value="${s_studentText }" placeholder="请输入~">
			...
			<table class="table table-striped table-bordered table-hover datatable">
				...
				<c:forEach  varStatus="i" var="record" items="${recordList }">
					<tr>
						...
						<td>${record.detail }</td>
						<td>
							<button class="btn btn-mini btn-success" type="button" onclick="javascript:window.location='record?action=preSave&recordId=${record.recordId }'">修改信息</button>
							<button class="btn btn-mini btn-danger" type="button" onclick="recordDelete(${record.recordId })">删除信息</button></td>
					</tr>
				</c:forEach>
				</tbody>
			</table>
